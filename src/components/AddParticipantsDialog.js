...
import { groupMembersForAdd } from '../data/mockData';

export default function AddParticipantsDialog({ subGroupData, onClose, onCreate }) {
  const [search, setSearch] = useState('');
  const [selected, setSelected] = useState([groupMembersForAdd[0], groupMembersForAdd[2]]);

  const filtered = groupMembersForAdd.filter(m =>
          ...
          {filtered.map(member => {
            ...
            return (
              ...
              >
                ...
                <span className="modal-item-name">{member.name}</span>
                {isSelected && (
